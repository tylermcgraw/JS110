function longestSentence(text) {
  let sentence = text.split(/[.!?]/).reduce((lastSentence, currentSentence) => {
    return currentSentence.split(' ').length > lastSentence.split(' ').length
      ? currentSentence : lastSentence;
  });
  if (sentence[0] === ' ') sentence = sentence.slice(1);
  console.log(sentence + text.slice(
    text.indexOf(sentence) + sentence.length,
    text.indexOf(sentence) + sentence.length + 1
  ) + "\n");

  let words = sentence.split(' ');
  console.log(`Length of the longest sentence is: ${words.length}\n`);
}
